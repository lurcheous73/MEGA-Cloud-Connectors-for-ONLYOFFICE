#!/usr/bin/env python3
"""BRIMSTONE: patch exact ONLYOFFICE CommunityServer 12.8 for MEGA S4 v4."""
...
def run_checked(cwd: pathlib.Path, *args: str) -> str:
    proc = subprocess.run(list(args), cwd=str(cwd), text=True,
...
"""
    replace_once(account, one_drive_block, mega_block,
...
"""
    save_replacement = """            authData = GetEncodedAccesToken(authData, prKey);
            ...
            // ProviderAccountDao will then store its own encrypted snapshot.
...
"""
    ...
    print("\nPASS - BRIMSTONE MEGA S4 v4 backend integration patch applied to CommunityServer 12.8.")
    return 0
