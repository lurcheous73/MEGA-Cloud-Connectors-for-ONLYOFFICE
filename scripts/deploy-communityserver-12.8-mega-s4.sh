...
REQUIRED_INSTALLER_COMMIT="a51eb933a2e895b70a73fdcf81e2e312e27054a1"
...
exact_preflight() {
    ...
    git -C "$REPO_ROOT" merge-base --is-ancestor "$REQUIRED_INSTALLER_COMMIT" "$head" \
        || fail "checkout does not contain validated installer commit $REQUIRED_INSTALLER_COMMIT"
    [ -z "$(git -C "$REPO_ROOT" status --porcelain)" ] || fail "connector worktree is not clean"
    echo "PASS: checkout contains validated installer commit ($head)"
