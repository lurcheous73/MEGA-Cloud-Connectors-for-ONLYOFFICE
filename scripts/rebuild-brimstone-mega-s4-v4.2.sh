...
# BRIMSTONE CUSTOM CODE — MEGA S4 v4.2 rebuild gate.
# v4.2 fixes post-save storage invalidation and bootstraps ONLYOFFICE auth
# for the server-side bucket discovery handler.
...
EXPECTED_LIVE_DLL_HASH="a5d6698434ef9a18909aa6a2b42657472d832396a918ae648bee2c63255133d2"

DIR="$DEV/module/ASC.Files.Thirdparty"
ORIG="$DIR/ASC.Files.Thirdparty.csproj"
BUILD="$DIR/ASC.Files.Thirdparty.BrimstoneMegaS4V42.Linux.csproj"
...
banner(){
  echo "============================================================"
  echo " BRIMSTONE MEGA S4 v4.2 - REBUILD GATE"
  echo "============================================================"
}
...
# v4.2 root-cause fixes.
grep -Fq 'DisposableHttpContext rejects null assignments' "$SRC/MegaS4ProviderInfo.cs" || fail "v4.2 storage invalidation fix marker missing"
! grep -Fq 'DisposableHttpContext.Current[key] = null;' "$SRC/MegaS4ProviderInfo.cs" || fail "illegal DisposableHttpContext null assignment still present"
grep -Fq 'ASC.Web.Studio.Global.Authenticate();' "$SRC/BrimstoneMegaS4Handler.cs" || fail "v4.2 handler auth bootstrap missing"
grep -Fq 'BRIMSTONE MEGA S4 handler error:' "$SRC/BrimstoneMegaS4Handler.cs" || fail "v4.2 handler JSON diagnostic boundary missing"

test -s "$PATCHER" || fail "v4 patcher missing"
test -s "$VERIFY" || fail "DLL verifier missing"
echo "PASS - reviewed BRIMSTONE v4.2 source present"
...
[[ "$LIVE_HASH" == "$EXPECTED_LIVE_DLL_HASH" ]] || fail "live DLL is not expected Brimstone v4 build: $LIVE_HASH"
echo "PASS - expected Brimstone v4 live DLL: $LIVE_HASH"

MEGA_ROWS="$(docker exec "$DB_CONTAINER" sh -lc 'mysql --batch --raw --skip-column-names -uroot -p"$MYSQL_ROOT_PASSWORD" onlyoffice -e "SELECT COUNT(*) FROM files_thirdparty_account WHERE LOWER(provider)=\"megas4\";"' 2>/dev/null | tr -d '\r')"
[[ "$MEGA_ROWS" == "0" ]] || fail "MegaS4 account rows exist: $MEGA_ROWS"
OLD_MAPS="$(docker exec "$DB_CONTAINER" sh -lc 'mysql --batch --raw --skip-column-names -uroot -p"$MYSQL_ROOT_PASSWORD" onlyoffice -e "SELECT COUNT(*) FROM files_thirdparty_id_mapping WHERE id LIKE \"megas4-%\" OR hash_id LIKE \"megas4-%\";"' 2>/dev/null | tr -d '\r')"
[[ "$OLD_MAPS" == "0" ]] || fail "old MEGA mapping rows exist: $OLD_MAPS"
echo "PASS - no MegaS4 rows or stale mappings before rebuild"
...
python3 "$PATCHER" "$DEV"
echo "PASS - exact upstream third-party project rebuilt with BRIMSTONE v4.2 source"
...
echo "=== RESTORE + BUILD ==="
docker run --rm \
  --name brimstone-megas4-v42-build \
  --entrypoint /bin/bash \
  -v "$DEV:/src" \
  ...
  -lc '
    set -euo pipefail
    PROJECT=module/ASC.Files.Thirdparty/ASC.Files.Thirdparty.BrimstoneMegaS4V42.Linux.csproj
...
echo
echo "=== CLR / BRIMSTONE v4.2 VALIDATION ==="
...
printf '%s\n' "$STRINGS" | grep -Fq 'DisposableHttpContext rejects null assignments'
echo 'PASS - Brimstone secret bridge type'
echo 'PASS - Brimstone bucket handler type'
echo 'PASS - Brimstone handler diagnostic marker'
echo 'PASS - Brimstone storage invalidation fix marker'
echo 'PASS - mapping-safe provider prefix embedded'
CHECK

echo
echo "============================================================"
echo " PASS - BRIMSTONE MEGA S4 v4.2 DLL BUILT AND VALIDATED"
echo "============================================================"
echo "Candidate: $NEW"
